	...
	public void inputCandidate() {
		...
		String s;
			while(scan.hasNextLine()){
				...
				String[] field=s.split(",");
				Candidate c=new Candidate(field[0],field[1],field[2],candidateList.size(),filepath);
...
public void normalAlloc(){         //allots all candidates seats, including ones who didn't get a seat after DS allocationns are over
	for(int i=0;i<candidateList.size();i++){
		...
		if(c.alloted()) continue;
		else{
			Vector<String> pref=c.getPrefList();
			for(int j=0;j<pref.size();j++){
				c.setPreference(j);
				String s=c.getCurrProg();
				int cat=Integer.parseInt(s.substring(s.length()-1,1));
				if(vpMap.get(s).getQuota()>vpMap.get(s).getCurrStr()){
						c.allotedseat();
						if(!c.getNat()){
...
public boolean seatdereservation(){       
	...
	for(String key:vpMap.keySet()){
		String cat=key.substring(key.length()-1,1);
